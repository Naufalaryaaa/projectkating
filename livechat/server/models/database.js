...
class Database {
    ...
    createConversation(conversationData, callback) {
        if (typeof conversationData === 'string' || typeof conversationData === 'number') {
            ...
            // New support: object with id, user_id, status
            const { id, user_id, status = 'active' } = conversationData;
            if (id) {
                // Create with custom ID
                this.db.run(
                    "INSERT OR IGNORE INTO conversations (id, user_id, status) VALUES (?, ?, ?)",
                    [id, user_id, status],
                    function(err) {
                        if (callback) callback(err, id);
                    }
    ...
    addKnowledge(content, embedding, metadata = {}, source = 'manual', category = 'general', callback) {
        const metadataStr = typeof metadata === 'string' ? metadata : JSON.stringify(metadata);
        const embeddingStr = Array.isArray(embedding) ? JSON.stringify(embedding) : embedding;

        this.db.run(
            `INSERT INTO knowledge_base (content, embedding, metadata, source, category)
             VALUES (?, ?, ?, ?, ?)`,
            [content, embeddingStr, metadataStr, source, category],
            function(err) {
                if (callback) callback(err, this ? this.lastID : null);
            }
        );
    ...
    getAllKnowledge(callback) {
        this.db.all(
            `SELECT id, content, category, source, created_at FROM knowledge_base ORDER BY created_at DESC`,
            callback
        );
    }

    // Get knowledge by category
    getKnowledgeByCategory(category, callback) {
        this.db.all(
            `SELECT id, content, category, source FROM knowledge_base WHERE category = ? ORDER BY created_at DESC`,
            [category],
            callback
        );
    }
    ...
    // Update knowledge
    updateKnowledge(id, content, embedding, category, callback) {
        const embeddingStr = Array.isArray(embedding) ? JSON.stringify(embedding) : embedding;

        this.db.run(
            `UPDATE knowledge_base SET content = ?, embedding = ?, category = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
            [content, embeddingStr, category, id],
            callback
        );
    }

    // Delete knowledge
    deleteKnowledge(id, callback) {
        this.db.run(
    ...
    // Semantic search - retrieve all knowledge with embeddings
    getAllKnowledgeWithEmbeddings(callback) {
        this.db.all(
            `SELECT id, content, embedding, category FROM knowledge_base WHERE embedding IS NOT NULL`,
            callback
        );
    }

    // Bulk insert knowledge
    bulkInsertKnowledge(items, callback) {
        const stmt = this.db.prepare(
            `INSERT INTO knowledge_base (content, embedding, metadata, source, category) VALUES (?, ?, ?, ?, ?)`
        );

        this.db.serialize(() => {
            items.forEach(item => {
                const embeddingStr = Array.isArray(item.embedding) ? JSON.stringify(item.embedding) : item.embedding;
                const metadataStr = typeof item.metadata === 'string' ? item.metadata : JSON.stringify(item.metadata || {});

                stmt.run(
                    [item.content, embeddingStr, metadataStr, item.source || 'bulk', item.category || 'general']
                );
            });
